...
class Company extends Component {
  ...
  submitHandler = async(event) => {
    ...
    let result=await Axios.post('https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Company.json',this.props.cmp)
    if(result.status===200){
        this.setState({saved:true})
        let List1 = [];
    const result1 = await Axios.get(
      "https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Company.json"
    );
  ...
  render = () => {
    ...
    return (
      <div className="container">
        ...
        <form onSubmit={this.submitHandler}>
          <div className="form-group">
            ...
            ></input>
            {this.state.comperr?<label style={{color:"Red"}}>Company name should be at least 4 characters.</label>:<div></div>}
          </div>
          <div className="form-group">
            ...
            ></input>
            {this.state.nameerr?<label style={{color:"Red"}}>Name should be at least 4 characters.</label>:<div></div>}
          </div>
          <div className="form-group">
      ...
      {/* {this.state.number==="+971"?<label style={{color:"Red"}}>Company name should be at least 4 characters.</label>:<div></div>} */}
            {/* <input
              type="text"
              className="form-control"
              id="ContactNum"
