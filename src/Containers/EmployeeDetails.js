    ...
    submit=async()=>{
        if(this.state.empEdit.nameerr===false && this.state.empEdit.emailerr===false && this.state.empEdit.designationerr===false && 
            this.state.empEdit.name!=="" && this.state.empEdit.Email!=="" && this.state.empEdit.Designation!==""
                ...
                let result=await Axios.put("https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Employee/"+this.state.Employee.key+".json",this.state.empEdit)
                if(result.status===200){
                    let List=[]
                    let result2= await Axios.get("https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Employee.json")
...
this.props.history.push({pathname:'/ListEmployee'})
  // console.log(this.props)
    ...
    delete=async()=>{
        let result=await Axios.delete("https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Employee/"+this.state.Employee.key+".json")
        if(result.status===200){
            let result2=await Axios.get("https://helpdesk-support-ticket-default-rtdb.asia-southeast1.firebasedatabase.app/Employee.json")
            if(result2.status===200){
                let list=[]
